...
$salt = '645154795';
$pass = '1533bc39358fc7aee495787043eafa83';
// End copy and paste
// Defaults
$projectName = 'git_test'; // Your project name
$email		 = 'kaner@rambler.ru'; // Email address you want pull notifcations to go to
...
if (isset($_GET['update'])) {
	...
	if ($pass === $check) {
		// what does the pull
		//chdir('/var/www/vamba.biz');
		//shell_exec('eval $(ssh-agent)');
		//shell_exec('ssh-add');
		//$output = shell_exec('cd /var/www/vamba.biz && git checkout master && git pull origin master');
		$output = shell_exec('cd /var/www/vamba.biz && git pull origin master 2>&1');
		if ( $output && $email ) {
